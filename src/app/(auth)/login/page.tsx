...
export default function LoginPage() {
  const { user, loading, exchangeKey, login, register, error, clearError } = useAuth();
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === "key") {
        await exchangeKey({ key: String(data.get("key") || "").trim().toUpperCase() });
        ...
        await register({
          qq,
          key: String(data.get("key") || "").trim().toUpperCase(),
          password: String(data.get("password") || ""),
          email: (String(data.get("email") || "").trim() || undefined) as string | undefined,
        });
      ...
      >
          <div className="login-card">
            ...
            <div className="tab-bar login-tabs" role="tablist">
              <TabButton active={tab === "key"} onClick={() => switchTab("key")} icon={KeyRound}>
                密钥
              </TabButton>
              <TabButton active={tab === "account"} onClick={() => switchTab("account")} icon={LogIn}>
                账号
              </TabButton>
              <TabButton active={tab === "register"} onClick={() => switchTab("register")} icon={UserPlus}>
                注册
              </TabButton>
            </div>
